...
export class JwtService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.SECRET_KEY = this.configService.getOrThrow<string>('JWT_SECRET_KEY');

    // 关键修改：从配置中读取过期时间，并确保它是数字 (例如 604800 秒 = 7天)
    // 使用 parseInt 确保值是数字，如果配置缺失或无效，默认设置为 1 天（86400秒）
    this.EXPIRATION_TIME = parseInt(this.configService.get<string>('JWT_EXPIRATION_TIME', '86400'), 10);

    // 检查密钥长度 (JJWT 推荐 32 字节，jsonwebtoken 没有强制要求，但长密钥更安全)
    if (this.SECRET_KEY.length < 32) {
      this.logger.warn('JWT_SECRET_KEY is too short. Please use a key of 32 characters or more for production.');
    }
  }
  ...
  async generateToken(createJwtDto: CreateJwtDto): Promise<string> {
    const payload = {
