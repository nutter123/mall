...
页面开发遵循Ant Design Pro的页面结构，通常包含以下部分：
...
权限配置文件位于 `src/access.ts`，用于定义用户权限：
...
export default function (initialState: { currentUser?: API.CurrentUser }) {
  ...
  return {
    ...
    canEditProduct: currentUser && (currentUser.access === 'admin' || currentUser.access === 'editor'),
    // 其他权限
  };
}
```
...
const ProductList: React.FC = () => {
  ...
  return (
    <div>
      <Access
        accessible={access.canEditProduct}
        fallback={<div>没有编辑权限</div>}
      >
...
const ProductList: React.FC = () => {
  ...
  return <div>{intl.formatMessage({ id: 'product.name' })}</div>;
};
```
...
确保Less和Tailwind CSS配置正确，检查是否有样式冲突：
...
检查权限配置文件 `src/access.ts` 是否正确，确保用户信息包含正确的权限字段。
